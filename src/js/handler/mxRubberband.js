...
function mxRubberband(graph) {
  if (graph != null) {
    ...
    this.forceRubberbandHandler = mxUtils.bind(this, function (sender, evt) {
      ...
      // MOUSE_DOWN + alt按键 --- 画矩形选区
      if (evtName == mxEvent.MOUSE_DOWN && this.isForceRubberbandEvent(me)) {
        //this.graph.container 画布区域
        var offset = mxUtils.getOffset(this.graph.container); //mxPoint {x: 0, y: 0}
        var origin = mxUtils.getScrollOrigin(this.graph.container); //mxPoint {x: 0, y: 0}
        origin.x -= offset.x;
        origin.y -= offset.y;
        this.start(me.getX() + origin.x, me.getY() + origin.y); //矩形左上角点
        me.consume(false); // 不阻止默认事件
      }
    });
...
mxRubberband.prototype.mouseDown = function (sender, me) {
  if (
    ...
    // 不阻止默认事件是为了，即使我们开始画矩形框，事件处理链仍然执行
    // 这在ExtJs中是必需的，例如隐藏当前的上下文菜单。在mouseMove中，我们将确保在进行橡皮筋绑扎时，我们不会选择任何东西。
    me.consume(false);
  }
};
