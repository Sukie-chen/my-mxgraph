 ...
 * Class: mxGraph
 * 继承mxEventSource
 * This is the main class of the package. To activate
 * panning and connections use <setPanning> and <setConnectable>.
 * 图形套圈需要创建实例<mxRubberband>(handler中)
 * 默认情况下，事件监听器都会添加致 <mouseListeners>
 *
 ...
 *
 * 这些事件监听器会按照上述的顺序执行 if they are enabled.
 *
 * Background Images:
 *
 * <setBackgroundImage>：设置背景图片宽高.
 * 如果宽高任一发生变动会触发 <view>'s <mxGraphView.validate>
 *
 * Cell Images:
 * 为单个图形设置背景时，需要先设置默认值
 * Possible shapes are
 ...
 * 尝试解析图像包中的图像样式值，
 * 并将mxImageBundle中定义的短数据uri转换为IETF的RFC
